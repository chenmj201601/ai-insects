...
from figure_utils import draw_figure_loss

# train.py
# 可以改变anchor的大小，注意训练和测试时要使用同样的anchor
...
if __name__ == '__main__':
    with fluid.dygraph.guard():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            timestring = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
            print('{}[TRAIN]epoch {}, loss: {}'.format(timestring, epoch, np.mean(losses)))
            ...
            for i, data in enumerate(valid_loader()):
                ...
                losses.append(loss.numpy())
            timestring = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
            print('{}[VALID]epoch {}, loss: {}'.format(timestring, epoch, np.mean(losses)))
            ...
            if (epoch % 10 == 0) or (epoch == MAX_EPOCH - 1):
                ...
                if count > 2:
                    ...
                    draw_figure_loss(sub_all_loss, sub_all_val_loss)
